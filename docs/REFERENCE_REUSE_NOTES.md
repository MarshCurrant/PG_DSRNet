...
| --- | --- | --- | --- | --- |
| PromptRR | LF/HF frequency prompts motivate explicit frequency supervision. The repo states code is still "come soon", so only the paper/README idea is usable. | Add `L_freq` on DSRNet `output_t` vs `target_t` with low-frequency blur and high-frequency residual terms. | Low for simplified loss; high for full diffusion/promptformer. | Yes, simplified loss only |
