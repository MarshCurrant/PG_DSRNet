...
class DSRNetModel(DSRNetBase):
    ...
    def get_loss(self, out_t, out_r, out_rr):
        ...
        loss_freq = out_t.new_zeros(())
        loss_prior = out_t.new_zeros(())
        if getattr(self.opt, 'lambda_freq', 0.0) > 0:
            loss_freq = self.loss_dic['freq'](out_t, self.target_t) * self.opt.lambda_freq
        if getattr(self.opt, 'lambda_prior', 0.0) > 0:
            loss_prior = self.loss_dic['prior'](
                out_t, self.target_t, out_r, self.target_r, self.input,
                prior_source=getattr(self.opt, 'prior_source', 'target_r')) * self.opt.lambda_prior
        return loss_t_pixel, loss_r_pixel, loss_t_vgg, loss_exclu, loss_recons, loss_freq, loss_prior

    def backward_G(self):
        self.loss_t_pixel, self.loss_r_pixel, self.loss_t_vgg, \
        self.loss_exclu, self.loss_recons, self.loss_freq, self.loss_prior = \
            self.get_loss(self.output_t, self.output_r, self.output_rr)
    ...
    def get_current_errors(self):
        ...
        if getattr(self, 'loss_freq', None) is not None:
            ret_errors['Freq'] = self.loss_freq.item()
        if getattr(self, 'loss_prior', None) is not None:
            ret_errors['Prior'] = self.loss_prior.item()
    ...
    def load(self, model):
        ...
        state_dict = _torch_load_compat(weight_path, map_location=self.device)
