...
def _wandb_configured():
    if os.environ.get('WANDB_DISABLED', '').lower() in ('1', 'true', 'yes'):
        return False
    return any(os.environ.get(key) for key in ('WANDB_PROJECT', 'WANDB_ENTITY', 'WANDB_NAME'))
...
def _get_wandb_run():
    ...
    try:
        ...
        if not _WANDB_WARNED:
            print('[w] wandb logging disabled: {}'.format(exc))
            _WANDB_WARNED = True
        return None

    init_kwargs = {
        'entity': os.environ.get('WANDB_ENTITY', '3268587895-fudan-university-school-of-management'),
        'project': os.environ.get('WANDB_PROJECT', 'dip'),
...
def write_loss(writer, prefix, avg_meters, iteration):
    ...
    for key in avg_meters.keys():
        ...
        scalar = _to_scalar(meter)
        if scalar is not None:
            wandb_payload[os.path.join(prefix, key)] = scalar
